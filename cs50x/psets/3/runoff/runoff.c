...
bool vote(int voter, int rank, string name)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        ...
        {
            // If yes, add index of candidate to preferences array
...
bool is_tie(int min)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        if (candidates[i].eliminated)
        {
            candidates_eliminated = candidates_eliminated + 1;
        }
        if ((candidates[i].votes == min) && (!candidates[i].eliminated))
        {
            candidates_tied = candidates_tied + 1;
        }
    }

    if (candidates_tied == (candidate_count - candidates_eliminated))
    {
        return true;
    }
    return false;
